...
function Dashboard({ onCreate, onView }: { onCreate: () => void; onView: () => void }) {
  ...
  <div className="mt-8 grid gap-6 xl:grid-cols-[1.55fr_1fr]"><section className="bg-card"><div className="flex items-center justify-between border-b border-border p-5"><div><h2 className="font-brand font-bold">Recent quotations</h2><p className="mt-1 text-xs text-muted-foreground">Latest client activity</p></div><Button variant="ghost" size="sm" onClick={onView}>View all <ChevronRight /></Button></div><QuoteTable /></section><section className="bg-primary p-6 text-primary-foreground"><p className="text-xs font-bold uppercase text-highlight">September performance</p><h2 className="mt-3 font-brand text-2xl font-bold">61.6% acceptance rate</h2><div className="mt-8 flex h-36 items-end gap-3">{[32,55,43,78,62,91,70].map((v,i)=><div key={i} className="flex-1 bg-primary-foreground/15"><div className="bg-highlight" style={{height:`${v}%`}} /></div>)}</div><div className="mt-4 flex justify-between text-[10px] opacity-70"><span>Week 1</span><span>Week 4</span></div><div className="mt-7 border-t border-primary-foreground/20 pt-5"><p className="text-xs opacity-70">Next expiring quotation</p><p className="mt-1 text-sm font-semibold">BABC-Q-0027 · in 4 days</p></div></section></div></div>;
}
